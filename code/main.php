...
foreach ($actionArray as $action) {
    switch ($action) {
        ...
        case "tallyVotes":
            $xmlRoot->appendChild(
                tallyVotes($dbConn, $xmlDoc, $_REQUEST["post_id"])
            );
        ...
        case "getComments":
            $xmlRoot->appendChild(
                getComments($dbConn, $xmlDoc, $_REQUEST["post_id"])
            );
        ...
        case "logs":
            $xmlRoot->appendChild(
                getLogs($dbConn, $xmlDoc, $_REQUEST["user_id"])
            );
        ...
        case "getUser":
            $xmlRoot->appendChild(
                getUser($dbConn, $xmlDoc, $_REQUEST["user_id"])
            );

            break;
        case "load_redstone_controls":
            $xmlRoot->appendChild(
                loadRedstoneControls($dbConn, $xmlDoc, $_REQUEST["user_id"])
            );
        ...
        case "load_fluid_modules":
            $xmlRoot->appendChild(
                getFluidLevels($dbConn, $xmlDoc, $_REQUEST["user_id"])
            );

            break;
        case "load_energy_modules":
            $xmlRoot->appendChild(
                getEnergyLevels($dbConn, $xmlDoc, $_REQUEST["user_id"])
            );

            break;
        case "remove_module":
            $xmlRoot->appendChild(
                removeModule($dbConn, $xmlDoc, $_REQUEST["token"])
            );

            break;
        case "redstone_event_dropdowns":
            $xmlRoot->appendChild(
                redstoneEventDropdowns($dbConn, $xmlDoc, $_REQUEST["user_id"])
            );

            break;
        case "get_redstone_sides":
            $xmlRoot->appendChild(
                getRedstoneSides($dbConn, $xmlDoc, $_REQUEST["token"])
            );
        ...
        case "create_redstone_event":
            $xmlRoot->appendChild(
                createRedstoneEvent(
                    $dbConn,
                    $xmlDoc,
                    $_REQUEST["storage_token"],
                    $_REQUEST["redstone_token"],
                    $_REQUEST["trigger_value"],
                    $_REQUEST["side"],
                    $_REQUEST["output_value"],
                    $_REQUEST["event_type"],
                    $_REQUEST["user_id"]
                )
            );
        ...
        case "load_redstone_events":
            $xmlRoot->appendChild(
                loadRedstoneEvents($dbConn, $xmlDoc, $_REQUEST["user_id"])
            );

            break;
        case "remove_event":
            $xmlRoot->appendChild(
                removeEvent($dbConn, $xmlDoc, $_REQUEST["event_id"])
            );
